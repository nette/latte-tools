...
class ExpressionParser
{
		...
		switch ($token->getType()) {
			...
			default:
				if ($token->test(/* Token::PUNCTUATION_TYPE */ 9, '[')) {
					...
					$token->test(/* Token::OPERATOR_TYPE */ 8, '=')
					&& (
	...
	public function parseHashExpression()
	{
		...
		while (!$stream->test(/* Token::PUNCTUATION_TYPE */ 9, '}')) {
			...
			if ($token = $stream->nextIf(/* Token::NAME_TYPE */ 5)) {
				...
				($token = $stream->nextIf(/* Token::STRING_TYPE */ 7))
	...
	public function parseSubscriptExpression($node)
	{
		...
		if ($token->getValue() == '.') {
			$token = $stream->next();
			if (
				$token->getType() == /* Token::NAME_TYPE */ 5
				||
				$token->getType() == /* Token::NUMBER_TYPE */ 6
				||
				($token->getType() == /* Token::OPERATOR_TYPE */ 8 && preg_match(Lexer::REGEX_NAME, $token->getValue()))
			) {
